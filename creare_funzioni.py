...
base_dir = trova_cartella_base()
cartella_funzioni = base_dir / "funzioni"

# 1. Creo la cartella "funzioni" se non esiste
cartella_funzioni.mkdir(exist_ok=True)
print(f"Cartella funzioni pronta in: {cartella_funzioni}")

# 2. Creo il file __init__.py (necessario per l'importazione)
(cartella_funzioni / "__init__.py").touch()

# 3. Creo il file utilita.py con le funzioni generali (file/csv/parametri)
testo_utilita = """
import os
from pathlib import Path
import numpy as np
...
"""
with open(cartella_funzioni / "utilita.py", "w") as f:
    f.write(testo_utilita.strip())


# 4. Creo il file astrometria.py con TUTTE le funzioni di analisi
testo_astrometria = """
import requests
...
"""
with open(cartella_funzioni / "astrometria.py", "w") as f:
    f.write(testo_astrometria.strip())

print("Setup Moduli completato con successo!")