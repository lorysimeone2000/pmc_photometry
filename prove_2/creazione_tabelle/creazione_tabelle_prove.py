...
def ottieni_coordinate_telescopio(nome_telescopio):
    # cerco il file delle posizioni all'interno del progetto
    file_posizioni = cerca_file_nel_progetto(BASE_DIR, "posizione_telescopi.txt")
    if file_posizioni:
        with open(file_posizioni, 'r') as f:
            # leggo tutte le righe rimuovendo gli spazi vuoti
            linee = [line.strip() for line in f.readlines() if line.strip()]

        # scorro il file per trovare il nome del telescopio
        for i, linea in enumerate(linee):
            if linea == nome_telescopio:
                try:
                    # estraggo i valori associati
    ...
    # se fallisce, ritorno i valori di default di sicurezza
    print(f"ATTENZIONE: Impossibile caricare le coordinate per {nome_telescopio}. Uso i valori di default.")
...
def scarica_tle_storici(tempo_astropy, username, password, cartella_output):
    ...
    # Impostiamo una finestra di -4 giorni fino a +1 giorno per assicurarci di beccare i MEO/GPS
    data_inizio_globale = data_osservazione - timedelta(days=4)
    data_fine_globale = data_osservazione + timedelta(days=1)

    nome_file = f"tle_storico_payload_{data_inizio_globale.strftime('%Y-%m-%d')}_to_{data_fine_globale.strftime('%Y-%m-%d')}.txt"
    ...
    print(f"Scaricando i TLE storici da Space-Track a blocchi di 1 giorno per evitare crash del server...")

    login_url = "https://www.space-track.org/ajaxauth/login"
    satelliti_univoci = {}

    with requests.Session() as session:
        # Effettuiamo il login una sola volta per l'intera sessione
        ...
        # Eseguiamo un ciclo giorno per giorno per non far andare in timeout il server
        giorno_corrente = data_inizio_globale
        while giorno_corrente < data_fine_globale:
            giorno_successivo = giorno_corrente + timedelta(days=1)
            str_inizio = giorno_corrente.strftime('%Y-%m-%d')
            str_fine = giorno_successivo.strftime('%Y-%m-%d')

            print(f" -> Scaricamento blocco {str_inizio} ...")

            # Query leggera limitata a sole 24 ore
            query_url = f"https://www.space-track.org/basicspacedata/query/class/gp_history/EPOCH/{str_inizio}--{str_fine}/OBJECT_TYPE/PAYLOAD/format/tle"
            ...
            if risposta_tle.status_code == 200:
                ...
                if "<html" in testo_risposta.lower()[:50]:
                    print("ERRORE: Space-Track ha bloccato il download (User Agreement o Rate Limit).")
                    return None

                # Estraiamo i TLE di questo blocco e aggiorniamo il dizionario
                linee = [l.strip() for l in testo_risposta.strip().split('\n') if l.strip()]
                for i in range(0, len(linee), 2):
                    if i + 1 < len(linee):
                        riga1 = linee[i]
                        riga2 = linee[i + 1]

                        # Sostituiamo o aggiungiamo il TLE. Procedendo in avanti nel tempo,
                        # il dizionario conserverà automaticamente il dato più aggiornato.
                        if riga1.startswith('1 ') and riga2.startswith('2 '):
                            norad_id = riga1[2:7].strip()
                            satelliti_univoci[norad_id] = f"{riga1}\n{riga2}\n"
            else:
                print(
                    f"    [!] Attenzione: Blocco {str_inizio} fallito con codice HTTP {risposta_tle.status_code}. Procedo al successivo.")

            # Avanziamo di un giorno per il ciclo successivo
            giorno_corrente = giorno_successivo

        # Alla fine del ciclo, salviamo tutto nel file
        if satelliti_univoci:
            with open(percorso_output, 'w') as f:
                for sat_str in satelliti_univoci.values():
                    f.write(sat_str)
            print(f"Download a blocchi completato! Salvati {len(satelliti_univoci)} TLE univoci.")
            return str(percorso_output)
        else:
            print("ERRORE CRITICO: Nessun TLE recuperato da Space-Track in nessun blocco.")
            return None
...
# 1. Inizializzo Skyfield (il download dei satelliti lo faccio prima del ciclo in base alla data)
ts = load.timescale()

# 2. Imposto le coordinate del mio telescopio
# estraggo i singoli valori di latitudine, longitudine e altitudine dalla funzione
lat_oss, lon_oss, alt_oss = ottieni_coordinate_telescopio('ASTRI 1')

# creo l'oggetto geografico wgs84 compatibile con la matematica vettoriale di skyfield
...
if __name__ == "__main__":
    ...
    # cerco la prima immagine disponibile per estrarre la data della nottata
        ...
        for n, percorso_file in enumerate(tqdm(file_list, desc=f"Fase 1 Run {run}"), 1):
            ...
            if 'ycentroid' in cols_order:
                ...
                if 'RAJ2000' in df_catalogate.columns:
                    ...
                    # procedo con il filtro solo se ho calcolato almeno un'orbita valida
