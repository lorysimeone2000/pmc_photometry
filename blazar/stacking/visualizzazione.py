...
nome_file_c = "coverage_map_mrk421_2026.fits"
nome_file_sum = "stacked_sum_mrk421_2026.fits"
...
if dir_tabelle_cat.exists():
    ...
    if tabelle_csv:
        ...
        if col_ra in df_catalogate.columns and col_dec in df_catalogate.columns:
            ...
            cat_coords = SkyCoord(ra=df_catalogate[col_ra].values * u.deg, dec=df_catalogate[col_dec].values * u.deg, frame='icrs')
...
# preparo la visualizzazione della mappa SNR
...
# passo l'oggetto 'im_snr' alla nuova colorbar
plt.colorbar(im_snr, label='Segnale / Rumore (Sigma)')

# aggiorno il titolo
plt.title(f'Mappa SNR Globale Mrk 421 (1.6x1.6 arcmin)\n(Valori in deviazioni standard)')
...
# salvo e mostro il risultato
plt.savefig('stacking_mrk421_snr_visualizzazione_2026.png')
# plt.show()