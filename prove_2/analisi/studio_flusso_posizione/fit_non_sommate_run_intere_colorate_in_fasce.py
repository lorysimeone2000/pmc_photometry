...
INDICE_IMMAGINE_RIFERIMENTO = 35  # Immagine 35 della Run 1 come riferimento
RUN_RIFERIMENTO = 1
NUM_FASCE = 3
...
# =============================================================================
# 2. CARICAMENTO DATI GLOBALE (TUTTE LE RUN)
# =============================================================================
...
for run in RUN_TO_ANALYZE:
    ...
    for f in tqdm(files_csv, leave=False):
        try:
            ...
            # Rimuovo le coordinate dal file globale per evitare che, durante il merge
            # con il file di riferimento, si creino colonne duplicate (xcentroid_x, xcentroid_y)
            # che fanno sparire la colonna 'xcentroid' originale.
...
nome_cartella_ref = f"tabelle_unite_run_{RUN_RIFERIMENTO}"
...
# Tengo la prima occorrenza per i dati fotometrici globali
...
print(f"Oggetti UNICI globali: {len(df_unique)}")

# B. MERGE CON LE COORDINATE DI RIFERIMENTO
# Uso 'left': mantengo tutte le stelle globali.
# Poiché ho rimosso xcentroid da df_unique nello Step 2,
# il merge aggiungerà 'xcentroid' pulito da df_coords_ref senza suffissi _x/_y.
...
data_out_ref = data_fit[~mask_in_ref].copy()  # Stelle grigie
...
# =============================================================================
# 6. FIT LINEARE (Svuotato - Il calcolo è spostato nel loop grafico)
# =============================================================================

# Ho rimosso il fit globale qui. Lo eseguo per ogni fascia direttamente nel grafico.
X_global_per_limits = data_fit['Mag'].values  # Mi serve solo per i limiti degli assi se necessario
...
if not data_in_ref.empty:
    ...
    for i, (r_limit, color) in enumerate(zip(radii_limits, colors_zones)):
        ...
        if len(subset) > 0:
            ...
            # Verifico di avere abbastanza punti per il fit
            if len(subset) > 2:
                ...
                try:
                    ...
                    label_fit_sub = rf'Fit Fascia {i + 1}: $m={m_fit_sub:.2f}$'
...
if not df_no_match.empty:
    # Creo una maschera per selezionare solo gli oggetti "No Match" che hanno coordinate valide
    # (quindi sono presenti nell'immagine di riferimento)
    mask_nm_in_ref = (
...
ax1.set_title(f'Calibrazione Globale (Rif: Run {RUN_RIFERIMENTO} Img {INDICE_IMMAGINE_RIFERIMENTO})', fontsize=14)
...
ax2.set_title(f"Rif: Run {RUN_RIFERIMENTO} Img {INDICE_IMMAGINE_RIFERIMENTO}\n(RA: {ra_val}, DEC: {dec_val})")
ax2.set_xlim(0, W)
ax2.set_ylim(0, H)

plt.tight_layout()
output_img = f"fit_globale_check_presenza_rif.png"
