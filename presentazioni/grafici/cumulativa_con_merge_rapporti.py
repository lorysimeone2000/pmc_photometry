...
# Calcolo il rapporto percentuale tra la linea rossa e la linea viola
rapporto_percentuale = np.zeros_like(counts_corr, dtype=float)
mask_non_zero = counts_cat != 0
rapporto_percentuale[mask_non_zero] = (counts_corr[mask_non_zero] / counts_cat[mask_non_zero]) * 100

# Imposto il valore a 100 per i casi in cui entrambe le linee sono a 0
mask_zero = (counts_cat == 0) & (counts_corr == 0)
rapporto_percentuale[mask_zero] = 100

# forzo a 100 i valori che superano tale soglia
rapporto_percentuale[rapporto_percentuale > 100] = 100

# Traccio la nuova linea del rapporto sul grafico
plt.plot(bin_centers, rapporto_percentuale,
         color='green',
         linestyle='--',
         linewidth=1,
         label='Ratio (%)')
...
plt.ylabel('Matching Ratio (%)', fontsize=14)
...
plt.tight_layout()
plt.savefig('sensibilità_rapporti_merge_con_taglio.png', dpi=300, bbox_inches='tight')
# plt.show()