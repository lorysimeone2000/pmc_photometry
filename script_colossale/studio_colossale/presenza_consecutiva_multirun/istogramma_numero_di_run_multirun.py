...
# estraggo label e numero_di_run
labels = df['label'].values
numero_di_run = df['numero_di_run'].values
...
print("\n=== Basic statistics ===")
print(f"Average run: {np.mean(numero_di_run):.2f}")
print(f"Median run: {np.median(numero_di_run):.2f}")
print(f"Min run: {np.min(numero_di_run)}")
print(f"Max run: {np.max(numero_di_run)}")
print(f"Standard deviation: {np.std(numero_di_run):.2f}")
...
# 4.1 ISTOGRAMMA DELLE numero_di_run
plt.figure(figsize=(12, 7))
plt.hist(numero_di_run, bins=range(min(numero_di_run), max(numero_di_run) + 2),
         color='skyblue', edgecolor='black', align='left', alpha=0.8)

plt.title("Distribution of object appearances", fontsize=14)
plt.xlabel("Number of run", fontsize=12)
...
scatter = plt.scatter(ra_list, dec_list, c=numero_di_run,
                      cmap='viridis', s=30, alpha=0.7,
                      edgecolors='black', linewidth=0.5)

cbar = plt.colorbar(scatter)
cbar.set_label('Number of run', fontsize=12)
...
stats_text += f'Min run: {np.min(numero_di_run)}\n'
stats_text += f'Max run: {np.max(numero_di_run)}\n'
stats_text += f'Mean run: {np.mean(numero_di_run):.1f}'
...
# istogramma
ax1.hist(numero_di_run, bins=range(min(numero_di_run), max(numero_di_run) + 2),
         color='skyblue', edgecolor='black', align='left', alpha=0.8)
ax1.set_title("Distribution of appearances", fontsize=12)
ax1.set_xlabel("Number of run", fontsize=10)
ax1.set_ylabel("Number of objects", fontsize=10)
ax1.grid(axis='y', linestyle='--', alpha=0.7)

# scatter plot
scatter2 = ax2.scatter(ra_list, dec_list, c=numero_di_run,
...
# oggetto con più numero_di_run
idx_max = np.argmax(numero_di_run)
print(f"\nObject with highest run ({numero_di_run[idx_max]}): {labels[idx_max]}")
