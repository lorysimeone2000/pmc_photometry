...
def tabella_catalogo(image_file_, magnitudine_massima):
    ...
    # Variabili globali prese dal contesto (inizializzate nel main loop)
...
if __name__ == "__main__":
    ...
    # --- RICERCA FILE PARAMETRI (RICORSIVA) ---
    ...
    # --- RICERCA DIRETTA DELLE CARTELLE RUN ---
    ...
    if len(found_folders) > 1:
        print(f"AVVISO: Trovate {len(found_folders)} cartelle '{nome_cartella_run}'. Uso la prima: {run_folder}")
    else:
        print(f"Cartella dati trovata: {run_folder.relative_to(BASE_DIR)}")

    # Cerca i file FITS dentro la cartella trovata
    ...
    # --- CARTELLA OUTPUT (MODIFICATA PER CERCARE 'tabelle' DINAMICAMENTE) ---
    # Cerchiamo la cartella 'tabelle' nell'albero delle directory (cerca ovunque, se la trova la usa)
    cartella_tabelle = cerca_cartella_nel_progetto(BASE_DIR, "tabelle")

    if cartella_tabelle is None:
        print(
            f"AVVISO: Cartella 'tabelle' non trovata in nessuna sottocartella di {BASE_DIR}. La creo nella root del progetto.")
    ...
    # Impostiamo il percorso finale dentro la cartella 'tabelle' trovata o creata
    ...
    i = 0
    print(f"Inizio elaborazione Fase 1 per {len(file_list)} files...")
    ...
    for percorso_file in tqdm(file_list, desc="Elaborazione Files"):
        ...
        if i == 1:
            ...
            # --- RICERCA HIPPARCO (RICORSIVA) ---
            file_hipparco = cerca_file_nel_progetto(BASE_DIR, "hipparco.fit")

            if file_hipparco is None:
                print(f"ERRORE CRITICO: Catalogo 'hipparco.fit' non trovato in nessuna sottocartella.")
                exit()

            # print(f"Uso catalogo Hipparco: {file_hipparco.relative_to(BASE_DIR)}")
    ...
    # =============================================================================
    # FASE 3: RICALCOLO FLUSSI CON APERTURA FISSA
    # =============================================================================
    ...
    for file_csv in tqdm(file_csv_list, desc="Ricalcolo Flussi"):
        ...
        # Fix path fits (Controllo ridondante di sicurezza)
