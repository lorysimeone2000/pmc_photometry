...
fig.suptitle(f'Magnitudine Integrata vs Parametri (Run {run} - Img {n_immagine})', fontsize=16)

plots_config = [
    ('area', axes[0], 'Area (pixel)'),
    ('kron_flux', axes[1], 'Kron Flux (conteggi)'),
    ('max_value', axes[2], 'Max Value (conteggi)')
]

for col_name, ax, x_label in plots_config:
    # 1. Singole Stelle (Blu)
    ax.scatter(df_singole[col_name], df_singole['Mag_Integrata'],
               s=size, c='blue', alpha=alpha, label='Singola Stella', edgecolors='k', linewidth=0.5)

    # 2. Stelle Multiple Integrate (Rosso)
    # Qui stiamo plottando la somma dei flussi delle componenti
    ax.scatter(df_multiple[col_name], df_multiple['Mag_Integrata'],
               s=size * 1.25, c='red', alpha=alpha, label='Multipla (Mag Integrata)', edgecolors='k', linewidth=0.5)

    # 3. Non Correlate (Arancione X)
    ax.scatter(df_no_corr_unique[col_name], y_no_corr,
               s=size, c='orange', marker='x', alpha=0.5, label='Non Correlate')

    ax.set_title(f'{x_label} vs Mag Integrata')
    ax.set_xlabel(x_label)
    ax.set_ylabel('Magnitudine Catalogata (Integrata)')
    ax.invert_yaxis()
    ax.set_xscale('log')
...
# --- VISUALIZZAZIONE DETTAGLIO FLUSSO ---
