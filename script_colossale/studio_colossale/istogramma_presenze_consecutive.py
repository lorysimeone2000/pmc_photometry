...
print("\n=== Basic statistics ===")
print(f"Average occurrences: {np.mean(occorrenze):.2f}")
print(f"Median occurrences: {np.median(occorrenze):.2f}")
print(f"Min occurrences: {np.min(occorrenze)}")
print(f"Max occurrences: {np.max(occorrenze)}")
...
stats_text += f'Min occurrences: {np.min(occorrenze)}\n'
stats_text += f'Max occurrences: {np.max(occorrenze)}\n'
stats_text += f'Mean occurrences: {np.mean(occorrenze):.1f}'
...
ax1.set_xlabel("Number of occurrences", fontsize=10)
...
ax2.set_title("Spatial distribution (colour = occurrences)", fontsize=12)
...
cbar2.set_label('Occurrences', fontsize=10)
...
print(f"\nObject with highest occurrences ({occorrenze[idx_max]}): {labels[idx_max]}")
