...
for n in range(len(lista_percorsi_csv_stelle_catalogate)):
    ...
    # ASSUMENDO che la colonna per la magnitudine si chiami 'Mag' o simile - MODIFICA SE NECESSARIO
    colonna_magnitudine = 'Mag'  # Cambia questo con il nome corretto della colonna (es. 'Vmag', 'Bmag', etc.)

    print(f"Cercata correlazione in {len(coords_trovate)} stelle")

    for idx_trovato in range(len(coords_trovate)):

        distanza_da_trovata = coords_catalogate.separation(coords_trovate[idx_trovato]) # per ogni stella trovata, cerco TUTTE le stelle catalogate

        mask_con_soglia = distanza_da_trovata <= soglia_correlazione # creo una maschera per le stelle catalogate entro la soglia
        indici_catalogate_vicine = np.where(mask_con_soglia)[0]
        ...
        if ha_corrispondenza:
            # MODIFICA CHIAVE: Trovo la stella più luminosa tra quelle vicine
            magnitudini_vicine = tbl_catalogate[colonna_magnitudine][indici_catalogate_vicine]

            # Trovo l'indice della stella con magnitudine minima (più luminosa)
            # tra quelle entro la soglia
            idx_piu_luminosa = indici_catalogate_vicine[np.argmin(magnitudini_vicine)]

            # dati dalla stella catalogata corrispondente (la più luminosa)
            for colname in tbl_catalogate.colnames:
                nuova_riga[colname] = tbl_catalogate[colname][idx_piu_luminosa]
