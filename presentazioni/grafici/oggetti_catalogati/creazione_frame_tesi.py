...
percorso_csv = cerca_file_nel_progetto(BASE_DIR, "stelle_tesi.csv")
...
for file_p in tqdm(file_parquet, desc="Analisi file"):
    try:
        ...
        # aggiungo 'Mag' alla lista delle colonne da estrarre
        tabella_p = pq.read_table(
            file_p,
            columns=['label', 'Mag', 'Mag_estratta', 'err_Mag_estratta', 'xcentroid', 'ycentroid', 'RA_centroid',
                     'DEC_centroid'],
        ...
        if tabella_p.num_rows > 0:
            ...
            # raggruppo per label ed estraggo gli indici delle righe con la Mag più bassa
            indici_min_mag = df_trovati.groupby('label')['Mag'].idxmin().dropna()

            # filtro il dataframe mantenendo esclusivamente i record con la magnitudine minima
            df_trovati = df_trovati.loc[indici_min_mag].copy()
            ...
            # estraggo la colonna Mag e la posiziono alla fine del dataframe rinominandola in Mag_vera
            df_trovati['Mag_vera'] = df_trovati.pop('Mag')
...
if dati_estratti:
    ...
    cartella_output = cerca_cartella_nel_progetto(BASE_DIR, "oggetti_catalogati")

    # Definisco il percorso per il nuovo file
    percorso_finale = cartella_output / "stelle_tesi_frame.csv"
