...
for i, chiave in enumerate(chiavi_flussi):
    ...
    plt.plot(x, p, color=colori_fit[i], linewidth=2, linestyle='-',
             label=f"Fit {nomi_legenda[i]} (mu={mu_fit:.4f}, sigma={sigma_fit:.4f}, chi_quad_rid={chi_quadro_ridotto:.2f})")

# inserisco il titolo comprensivo di tutti i riferimenti
plt.title("Distribuzione delle Differenze Assolute: Senza Correzioni vs Pre-TFA vs Post-TFA \n intervallo fit di 1 sigma",
...
plt.savefig(nuova_sottocartella / "dispersione_istogrammi_sovrapposti_assoluta.jpg", dpi=300)
# plt.show()
