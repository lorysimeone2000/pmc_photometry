...
for flusso in FLUSSI_DA_ANALIZZARE:
    ...
    if len(df_fit_curr) > 2:
        ...
        try:
            ...
            # eseguo il mio ciclo per calcolare e tracciare un fit per ogni run
            for run_id in RUN_TO_ANALYZE:
                # isolo i miei dati per la run corrente direttamente dal dataframe grezzo
                ...
                # eseguo il mio raggruppamento per label applicando le direttive del dizionario globale
                ...
                # applico il mio filtro numerico mantenendo le mie condizioni base
                mask_valid = (
                ...
                if len(df_run_final) > 2:
                    # estraggo i miei parametri specifici per il fit di questa run
                    ...
                    y_model = modello_lineare(X_run, m_fit, q_fit)
                    chi2 = np.sum(((Y_log_run - y_model) / sigma_log_run) ** 2)
                    dof = len(X_run) - 2
                    ...
                    # aggiungo il mio errore sui parametri nella label rispettando la base originale
                    label_fit = (
                        rf'Fit Run {run_id}: log(F)=({m_fit:.2f}$\pm${err_m:.2f})M + ({q_fit:.2f}$\pm${err_q:.2f}) '
            ...
            # configuro il mio grafico generale
            plt.yscale('log')
            ...
            print(f"Errore durante l'esecuzione del plot e dei fit per {flusso}: {e}")
    else:
        print(f"Non ho abbastanza punti validi per plottare l'analisi di {flusso}.")
