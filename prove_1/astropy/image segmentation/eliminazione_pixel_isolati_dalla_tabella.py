...
#image_file = "/home/lorysimeone/tesi_magistrale/20250120_run1/20250120_213319.fits"  # prima immagine run 1
...
try:
    ...
    print("ATTENZIONE: File dei parametri non trovato. Usando valori di default.")
    parametri = {'fwhm': 3.0, 'size': 5, 'threshold_sigma': 1.5, 'threshold_assoluta': 0.5, 'pixel': 5}
...
threshold_assoluta = parametri['threshold_assoluta']

finder = SourceFinder(npixels=n, progress_bar=True) # inserisco n. minimo di pixel
segment_map = finder(convolved_data, threshold_assoluta)
print(segment_map)
...
'''
# se applico il filtraggio, non devo generare le aperture prima
...
# Studio di segment_map

print("Ristrutturazione della tabella senza i pixel isolati")

print(segment_map.data)



print("=== FILTRAGGIO DIRETTO SUL CATALOGO ===")

# soglia da verificare
soglia = 2.5 # soglia assoluta usata per i pixel isolati
soglia_relativa = 0.05 # rispetto al massimo

# creo il catalogo COMPLETO
cat_completo = SourceCatalog(data, segment_map, convolved_data=convolved_data)
tbl_completo = cat_completo.to_table()
...
for i, sorgente in enumerate(tbl_completo):
    ...
    '''if label==189:
        print(f"Per esempio la sorgente {label} ha: \n {valori_originali}")
        print(f"Coordinate: {sorgente['xcentroid']} , {sorgente['ycentroid']}")
        print(f"Ha {len(valori_originali)} elementi")'''
