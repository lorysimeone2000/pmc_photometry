...
for size_val in SIZES_TO_TEST:
    ...
    # Variabile per tracciare l'ultimo FWHM con problemi nell'alone
    last_halo_fwhm = 0.0
    ...
    for i, fwhm_val in enumerate(FWHM_RANGE):
        ...
        if len(tbl) > 0:
            ...
            tbl_corr = tbl_corr[tbl_corr['Mag'] < 11]

            valore_corr = max(0, len(tbl_corr) - 400)
            ...
            if len(tbl_non_corr) > 0:
                ...
                # Se trovo un falso positivo nell'alone, aggiorno il contatore
    ...
    # Disegna la linea verticale se è stato trovato un limite
    if last_halo_fwhm > 0:
