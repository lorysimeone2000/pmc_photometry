...
print("\n--- ELABORAZIONE GRAFICO 1: FIT GLOBALE ---")
plt.figure(figsize=(12, 9))

if len(df_fit_clean) > 2:
    X_all_g = df_fit_clean['Mag'].values
    Y_all_g = df_fit_clean['media_flusso_fisso_max_run'].values
    Err_all_g = df_fit_clean['std_flusso_fisso_max_run'].values
    ...
    plt.title(f"Calibrazione Fotometrica Globale Binnata (Run {RUN_TO_ANALYZE})", fontsize=14)
    plt.grid(True, which="both", ls="-", alpha=0.2)
    plt.legend(fontsize=11, loc='best')
    plt.tight_layout()

    out_file_globale = "fit_globale_binned_media_non_pesata.png"
...
print("\n--- ELABORAZIONE GRAFICO 2: FIT PER SINGOLA RUN ---")
plt.figure(figsize=(12, 9))
...
for idx_run, run_id in enumerate(sorted(df_fit_clean['run_origin'].unique())):
    ...
    if len(df_run) > 2:
        ...
        Y_all = df_run['media_flusso_fisso_max_run'].values
        Err_all = df_run['std_flusso_fisso_max_run'].values
...
plt.xlabel("Magnitudine Catalogo (Mag)", fontsize=12)
plt.ylabel("Media Flusso Fisso Max Run (ADU)", fontsize=12)
plt.title(f"Calibrazione Fotometrica Globale Binnata per Singola Run", fontsize=14)
plt.grid(True, which="both", ls="-", alpha=0.2)
plt.legend(fontsize=10, loc='best')
plt.tight_layout()

out_file_run = "fit_globale_binned_per_run_media_non_pesata.png"
