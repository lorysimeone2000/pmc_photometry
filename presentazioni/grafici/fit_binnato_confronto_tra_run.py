...
def trova_cartella_base(nome_target="pmc_photometry"):
    ...
    for parent in [path_corrente] + list(path_corrente.parents):
        if parent.name == nome_target:
            return parent
    print(f"ATTENZIONE: Cartella '{nome_target}' non trovata nell'albero. Uso la mia directory dello script.")
...
def cerca_cartella_nel_progetto(base_dir, nome_cartella_esatto):
    # cerco la mia cartella specificata nel percorso del mio progetto
...
# definisco il mio flusso esatto che voglio analizzare applicando la correzione additiva e la decorrelazione globale
...
for run in RUN_TO_ANALYZE:
    ...
    print(f"Run {run}: Trovati {len(files_parquet)} file. Caricamento in corso...")
...
# tengo esclusivamente la mia prima occorrenza per ogni label (quella con Mag minore)
df_match = df_match_sorted.drop_duplicates(subset=['label'], keep='first').copy()

# rimuovo i miei duplicati dai miei dati non catalogati basandomi unicamente sulla label
...
# =============================================================================
# 3. FIT BINNATI SEPARATI E PLOTTING CICLICO
# =============================================================================
...
for flusso in FLUSSI_DA_ANALIZZARE:
    ...
    if len(df_fit_curr) > 2:
        # estraggo i miei dati X globali per i limiti del plot
        ...
        try:
            ...
            for run_id in RUN_TO_ANALYZE:
                ...
                df_run_match = df_match_run_sorted.drop_duplicates(subset=['label'], keep='first').copy()

                # rimuovo i miei dati saturi per la singola run
                if 'saturazione' in df_run_match.columns:
                    mask_sature_run = df_run_match['saturazione'] == True
                ...
                if len(df_run_final) > 2:
                    ...
                    plt.errorbar(
                        ...
                        fmt='o', markersize=2, color=colore_linea, ecolor=colore_linea, capsize=1, alpha=0.6,
