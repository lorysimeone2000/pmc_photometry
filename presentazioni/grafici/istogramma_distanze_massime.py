...
if len(ref_x) > 0:
    ...
    plt.scatter(posizioni_vere_pixel[:, 0], posizioni_vere_pixel[:, 1],
                c=colors, s=36, alpha=1, edgecolor='magenta', label='Catalogued')
    cmap = plt.cm.viridis_r
    norm = plt.Normalize(vmin=magnitudini.min(), vmax=magnitudini.max())
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=plt.gca(), label='Catalogued magnitudes')
    ...
    plt.plot(ref_x[0], ref_y[0], 'rx', markersize=15, markeredgewidth=2, label='Start', zorder=11)
    plt.plot(ref_x[-1], ref_y[-1], 'gx', markersize=15, markeredgewidth=2, label='End', zorder=11)

    # 3. Aperture (Cerchi gialli) attorno alla stella trovata nel frame di riferimento

    if wcs_image_ref:
        aperture.plot(color='yellow', lw=1.5, alpha=0.8, label='Correlation region')
...
if tutte_le_distanze_massime:
    ...
    # creo l'istogramma impostando la larghezza a 0.55\textwidth in un documento A4
    plt.figure(figsize=(5.5, 4))

    # imposto l'istogramma e aggiungo i valori formattati e tradotti per la legenda
    ...
    # traduco in inglese britannico formale gli assi e imposto i fontsize per il LaTeX
    plt.xlabel('Maximum distance (arcseconds)', fontsize=10)
    ...
    # mostro la legenda rimpicciolendo il testo per farlo calzare nella figura più piccola
    plt.legend(fontsize=8)

    # salvo l'istogramma nella directory corrente ad alta risoluzione
    plt.savefig("istogramma_distanze_massime.png", dpi=300, bbox_inches='tight')
    # plt.show()
else:
