...
cartella_tabelle = BASE_DIR / "tabelle_blazar_e_non_blazar" / "tabelle_catalogo"
    ...
    for run_name, file_list in runs_nel_giorno.items():
        ...
        for percorso_file_fits in file_list:
            ...
            if i == 1:  # chiamo il sito una volta sola sulla prima immagine della run
                ...
                # imposto un timeout più lungo per le mie richieste (es. 120 secondi invece di 60)
                vizier.TIMEOUT = 120

                # implemento un ciclo di retry per aggirare i sovraccarichi temporanei del database
                tentativi_massimi = 5
                attesa = 15
                ...
                for tentativo in range(tentativi_massimi):
                    try:
                        ...
                        break  # esco dal ciclo se la richiesta va a buon fine
                    except Exception as e:
                        if tentativo < tentativi_massimi - 1:
                            print(
                                f"Timeout o errore di connessione VizieR. Attendo {attesa} secondi e riprovo ({tentativo + 1}/{tentativi_massimi})...")
                            time.sleep(attesa)
                            # aumento l'attesa progressivamente per i tentativi successivi
                            attesa += 10
                        else:
                            print("Numero massimo di tentativi raggiunto. Impossibile connettersi a VizieR.")
                            raise e
            ...
            filename = output_dir / f'{run_name}_stelle_catalogate_immagine_{i:03d}.csv'
