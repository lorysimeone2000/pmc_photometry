...
for run in RUN_TO_ANALYZE:
    ...
    if path_cartella is None:
        ...
        continue
    else: print(f"cartella trovata in {path_cartella}")
    ...
    for f in tqdm(files_csv, leave=False):
        try:
            # Aggiungo alla lista tutte le colonne dei flussi che mi servono successivamente per il filtro
            cols_needed = [
...
if len(df_fit_clean) > 2:
    ...
    if len(df_fit_clean) > 2:
        # Dati X e Y
        X = df_fit_clean['Mag'].values
        Y_flux = df_fit_clean['media_flusso_raggio_fisso_doppio'].values
        Y_log = np.log10(Y_flux)

        # Errori
        sigma_flux = df_fit_clean['std_flusso_raggio_fisso_doppio'].values
        # Propagazione errore sul logaritmo
        sigma_log = (1 / np.log(10)) * (sigma_flux / Y_flux)
        ...
        if not df_sature.empty:
            # Mi assicuro che abbiano dati validi per il plot
            mask_sat_valid = (df_sature['media_flusso_raggio_fisso_doppio'] > 0) & (df_sature['Mag'].notna())
            df_sature_plot = df_sature[mask_sat_valid]

            if not df_sature_plot.empty:
                plt.scatter(
                    df_sature_plot['Mag'],
                    df_sature_plot['media_flusso_raggio_fisso_doppio'],
                    s=40, c='red', marker='x', linewidth=1,
                    label=f'Sature (Escluse) ({len(df_sature_plot)})', zorder=20
                )
        ...
        if not df_no_match.empty:
            ...
            # Filtro quelli con flusso valido
            df_no_match_plot = df_no_match[df_no_match['media_flusso_raggio_fisso_doppio'] > 0]

            if not df_no_match_plot.empty:
                plt.scatter(
                    np.full(len(df_no_match_plot), mag_fittizia),
                    df_no_match_plot['media_flusso_raggio_fisso_doppio'],
                    s=40, c='orange', marker='D', edgecolors='black', alpha=0.8,
                    label=f'NON Catalogati ({len(df_no_match_plot)})', zorder=10
                )
                # Aggiungo annotazione
                plt.annotate("Mag Fittizia",
                             xy=(mag_fittizia, np.mean(df_no_match_plot['media_flusso_raggio_fisso_doppio'])),
                             xytext=(mag_fittizia, np.max(df_no_match_plot['media_flusso_raggio_fisso_doppio']) * 1.5),
                             arrowprops=dict(facecolor='black', arrowstyle='->'),
                             ha='center')
        ...
        plt.gca().invert_xaxis()
        plt.xlabel("Magnitudine Catalogo (Mag)", fontsize=12)
        plt.ylabel("Media Flusso Fisso Doppio Max Run (ADU)", fontsize=12)
        plt.title(f"Calibrazione Fotometrica Globale (Run {RUN_TO_ANALYZE})", fontsize=14)
        plt.grid(True, which="both", ls="-", alpha=0.2)
        plt.legend(fontsize=11, loc='best')
        plt.tight_layout()
        ...
        # Salvataggio
        out_file = "fit_globale_flusso_doppio_media_style.png"
        plt.savefig(out_file, dpi=300)
        print(f"Grafico salvato: {out_file}")
        plt.show()
        ...
        if len(df_fit_clean) > 2:
            # Dati X e Y
            X = df_fit_clean['Mag'].values
            Y_flux = df_fit_clean['media_flusso_intera_segmentazione'].values
            Y_log = np.log10(Y_flux)

            # Errori
            sigma_flux = df_fit_clean['std_flusso_intera_segmentazione'].values
            # Propagazione errore sul logaritmo
            sigma_log = (1 / np.log(10)) * (sigma_flux / Y_flux)
            ...
            if not df_sature.empty:
                # Mi assicuro che abbiano dati validi per il plot
                mask_sat_valid = (df_sature['media_flusso_intera_segmentazione'] > 0) & (df_sature['Mag'].notna())
                df_sature_plot = df_sature[mask_sat_valid]

                if not df_sature_plot.empty:
                    plt.scatter(
                        df_sature_plot['Mag'],
                        df_sature_plot['media_flusso_intera_segmentazione'],
                        s=40, c='red', marker='x', linewidth=1,
                        label=f'Sature (Escluse) ({len(df_sature_plot)})', zorder=20
                    )
            ...
            if not df_no_match.empty:
                ...
                # Filtro quelli con flusso valido
                df_no_match_plot = df_no_match[df_no_match['media_flusso_intera_segmentazione'] > 0]

                if not df_no_match_plot.empty:
                    plt.scatter(
                        np.full(len(df_no_match_plot), mag_fittizia),
                        df_no_match_plot['media_flusso_intera_segmentazione'],
                        s=40, c='orange', marker='D', edgecolors='black', alpha=0.8,
                        label=f'NON Catalogati ({len(df_no_match_plot)})', zorder=10
                    )
                    # Aggiungo annotazione
                    plt.annotate("Mag Fittizia",
                                 xy=(mag_fittizia, np.mean(df_no_match_plot['media_flusso_intera_segmentazione'])),
                                 xytext=(mag_fittizia, np.max(df_no_match_plot['media_flusso_intera_segmentazione']) * 1.5),
                                 arrowprops=dict(facecolor='black', arrowstyle='->'),
                                 ha='center')
            ...
            plt.gca().invert_xaxis()
            plt.xlabel("Magnitudine Catalogo (Mag)", fontsize=12)
            plt.ylabel("Media Flusso intera segmentazione (ADU)", fontsize=12)
            plt.title(f"Calibrazione Fotometrica Globale (Run {RUN_TO_ANALYZE})", fontsize=14)
            plt.grid(True, which="both", ls="-", alpha=0.2)
            plt.legend(fontsize=11, loc='best')
            plt.tight_layout()
            ...
            # Salvataggio
            out_file = "fit_globale_flusso_intera_segmentazione_media_style.png"
            plt.savefig(out_file, dpi=300)
            print(f"Grafico salvato: {out_file}")
            plt.show()
            ...
            if len(df_fit_clean) > 2:
                # Dati X e Y
                X = df_fit_clean['Mag'].values
                Y_flux = df_fit_clean['media_flusso_kron_intera_segmentazione'].values
                Y_log = np.log10(Y_flux)

                # Errori
                sigma_flux = df_fit_clean['std_flusso_kron_intera_segmentazione'].values
                # Propagazione errore sul logaritmo
                sigma_log = (1 / np.log(10)) * (sigma_flux / Y_flux)
                ...
                if not df_sature.empty:
                    # Mi assicuro che abbiano dati validi per il plot
                    mask_sat_valid = (df_sature['media_flusso_kron_intera_segmentazione'] > 0) & (df_sature['Mag'].notna())
                    df_sature_plot = df_sature[mask_sat_valid]

                    if not df_sature_plot.empty:
                        plt.scatter(
                            df_sature_plot['Mag'],
                            df_sature_plot['media_flusso_kron_intera_segmentazione'],
                            s=40, c='red', marker='x', linewidth=1,
                            label=f'Sature (Escluse) ({len(df_sature_plot)})', zorder=20
                        )
                ...
                if not df_no_match.empty:
                    ...
                    # Filtro quelli con flusso valido
                    df_no_match_plot = df_no_match[df_no_match['media_flusso_kron_intera_segmentazione'] > 0]

                    if not df_no_match_plot.empty:
                        plt.scatter(
                            np.full(len(df_no_match_plot), mag_fittizia),
                            df_no_match_plot['media_flusso_kron_intera_segmentazione'],
                            s=40, c='orange', marker='D', edgecolors='black', alpha=0.8,
                            label=f'NON Catalogati ({len(df_no_match_plot)})', zorder=10
                        )
                        # Aggiungo annotazione
                        plt.annotate("Mag Fittizia",
                                     xy=(mag_fittizia, np.mean(df_no_match_plot['media_flusso_kron_intera_segmentazione'])),
                                     xytext=(mag_fittizia,
                                             np.max(df_no_match_plot['media_flusso_kron_intera_segmentazione']) * 1.5),
                                     arrowprops=dict(facecolor='black', arrowstyle='->'),
                                     ha='center')
                ...
                plt.gca().invert_xaxis()
                plt.xlabel("Magnitudine Catalogo (Mag)", fontsize=12)
                plt.ylabel("Media Flusso kron intera segmentazione (ADU)", fontsize=12)
                plt.title(f"Calibrazione Fotometrica Globale (Run {RUN_TO_ANALYZE})", fontsize=14)
                plt.grid(True, which="both", ls="-", alpha=0.2)
                plt.legend(fontsize=11, loc='best')
                plt.tight_layout()
                ...
                # Salvataggio
                out_file = "fit_globale_flusso_kron_intera_segmentazione_media_style.png"
                plt.savefig(out_file, dpi=300)
                print(f"Grafico salvato: {out_file}")
                plt.show()
