...
for flusso in FLUSSI_DA_ANALIZZARE:
    ...
    if len(df_fit_curr) > 2:
        ...
        try:
            ...
            # =============================================================================
            # creo il mio grafico con dimensioni adatte per una colonna da 0.85\textwidth
            plt.figure(figsize=(8, 5))

            # disegno i miei punti originali (Blu con barre errore chiare)
            plt.errorbar(
                X, Y_flux, yerr=sigma_flux,
                fmt='o', markersize=.5, color='blue', ecolor='lightblue', alpha=0.7,
                label=f'Valid catalogued sources ({len(X)})'
            )

            # disegno i miei bin usati per il fit (Verdi)
            plt.errorbar(
                X_binned, Y_binned, yerr=Err_binned,
                fmt='o', markersize=2, color='green', ecolor='green', capsize=1, alpha=0.9,
                label=f'Binned fit ({len(X_binned)})', zorder=15
            )
            ...
            if not df_sature_plot.empty:
                plt.scatter(
                    ...
                    s=30, c='red', marker='x', linewidth=1,
                    label=f'Saturated sources (excluded) ({len(df_sature_plot)})', zorder=20
                )
            ...
            if not df_no_match_plot.empty:
                ...
                plt.scatter(
                    ...
                    s=25, c='orange', marker='D', edgecolors='black', alpha=0.8,
                    label=f'Uncatalogued sources ({len(df_no_match_plot)})', zorder=10
                )
            ...
            # aggiungo il mio errore sui parametri nella label rimuovendo il chi quadro
            label_fit = rf'Binned fit: log(F)=({m_fit:.2f}$\pm${err_m:.2f})M + ({q_fit:.2f}$\pm${err_q:.2f})'

            plt.plot(x_plot, 10 ** y_plot_log, 'g--', linewidth=2, label=label_fit, zorder=16)

            # configuro il mio grafico con font adatti
            plt.yscale('log')
            ...
            plt.xlabel("Catalogue magnitude", fontsize=14)
            plt.ylabel("Instrumental flux (ADU)", fontsize=14)
            plt.grid(True, which="both", ls="-", alpha=0.2)

            # configuro la legenda
            plt.legend(fontsize=12, loc='best')

            # salvo preservando i margini
            plt.tight_layout()

            # salvo il mio grafico
            plt.savefig(f"fit_bin_pesi_uguali.png", dpi=300, bbox_inches='tight')
